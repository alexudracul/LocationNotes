...
var folders: [Folder] {
    ...
    let folders_array = try? CoreDataManager.sharedInstance.managedObjectContext.fetch(request)
    
    if folders_array != nil {
...
var notes: [Note] {
    ...
    let notes_array = try? CoreDataManager.sharedInstance.managedObjectContext.fetch(request)
